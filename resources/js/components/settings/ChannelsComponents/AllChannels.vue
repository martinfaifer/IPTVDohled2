    ...
    <div class="body-2">
        ...
        >
                                ...
                                <v-card-text>
                                    ...
                                    >
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field
                                                v-model="apiUrl"
                                                label="Url kanálu z dokumentace"
                                            ></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row
            ...
            <template v-slot:item.api="{ item }">
                <v-icon v-if="item.api != '1'" color="red">mdi-close</v-icon>
                <v-icon v-else color="green">mdi-check</v-icon>
            </template>

            <!-- dohled volume -->
            <template v-slot:item.dohledVolume="{ item }">
                <v-icon v-if="item.dohledVolume != '1'" color="red"
                    >mdi-close</v-icon
                >
                <v-icon v-else color="green">mdi-check</v-icon>
            </template>
            ...
            <template v-slot:item.noMonitor="{ item }">
                <v-icon :color="getMonitorStatus(item.noMonitor)" dark>{{
                    item.noMonitor
                }}</v-icon>
            </template>
                ...
                <v-card>
                    <v-card-text>
                        ...
                        <v-row>
                            <v-col cols="12" sm="12" md="12">
                                <v-text-field
                                    v-model="editdat.dokumentaceUrl"
                                    label="Url kanálu z dokumentace"
                                ></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row v-show="editdat.dokumentaceUrl != null">
                            <v-col cols="12" sm="12" md="12">
                                <v-autocomplete
                                    v-model="editdat.api"
                                    :items="apis"
        ...
        <v-row justify="center">
            <v-dialog v-model="alertDialog" persistent width="1000">
                <v-card>
                    <v-card-title class="headline"
                        >Nastavení, kdy kanál má posílat Alerting</v-card-title
                    >
                    <v-card-text>
                        <v-row v-if="alertData != false">
                            <span class="red--text">
                                <strong
                                    ...
                                    od {{ alertData.start_dohled }} do
                                    {{ alertData.end_dohled }}</strong
                                >
                            </span>
                        </v-row>
                        <v-row class="mt-2">
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            { text: "URL", value: "url" },
            { text: "Dokumentace URL", value: "dokumentaceUrl" },
            { text: "Externi API", value: "api" },
