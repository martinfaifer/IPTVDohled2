      ...
      <div v-if="value === 'Výpadky'">
        <div>
          ...
          <p class="text-center">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon @click="cusotmErrorChartDialog = true" color="primary" v-on="on">mdi-magnify</v-icon>
              </template>
              <span>Vlastní výběr</span>
            </v-tooltip>
          </p>
          ...
          <v-card-text>
            <v-row>
              <v-row>
                ...
                <v-date-picker locale="cs" first-day-of-week="1" label="Do" v-model="doDate"></v-date-picker>
              </v-row>

              <v-row>
                <v-time-picker format="24hr" class="ml-8 mr-8" v-model="odCas"></v-time-picker>
                <v-time-picker format="24hr" v-model="doCas"></v-time-picker>
              </v-row>
            </v-row>
          </v-card-text>
          <v-card-actions>
            ...
            <v-btn color="green darken-1" text @click="sendData()">Zobrazit</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
      ...
      <v-dialog v-model="cusotmBitrateChartDialog" width="700" persistent>
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="red darken-1" text @click="closeDialog()">Zavřít</v-btn>
            <v-btn color="green darken-1" text @click="sendBitrateData()">Zobrazit</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
      ...
      <v-dialog v-model="cusotmErrorChartDialog" width="700" persistent>
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="red darken-1" text @click="closeDialog()">Zavřít</v-btn>
            <v-btn color="green darken-1" text @click="sendErrorteData()">Zobrazit</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
